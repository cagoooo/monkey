...
| Emoji | 主題 | 物理 | 視覺特色 |
|---|---|---|---|
| 🏫 | **校園決鬥** | 標準重力 | 黑板綠漸層 + 紅磚教學樓 / 灰水泥 / 黃涼亭 |
| 🎄 | **節慶煙火** | 重力 0.22 + 大風 ±0.12 | 紅金漸層 + **60 顆飄雪**（左右擺動）+ 金色拖尾 |
| 🌋 | **火山熔岩** | 重力 0.30（最重）| 火紅漸層 + **40 顆上升火星**（紅→橘漸暗）+ 火橘拖尾 |

#### bananaTrail 主題化（每主題不同顏色 + 線寬）
- 城市：白半透 / 1.5px
- 太空：藍光 / 2px
- 深海：淺藍泡沫 / 2px
- 校園：粉筆白 / 1.5px
...
> ⚠️ ACID 強酸保留特殊綠色覆蓋（道具優先級高於主題）

#### 新增 BackgroundLayer kinds
- `snow` — 60 顆飄雪從上往下 + 左右擺動
- `lavaSparks` — 40 顆火星從下往上 + 顏色漸變（紅→橘→暗）+ 不透明度衰減

### 🐵 C5 Sprite 動畫輕量版

- **Idle 呼吸**：站著沒事時 monkey 會輕微上下浮動（`sin(time/600) × 0.8px`）
  - 條件：`!isThrowing && !isWinner && !isDead && !isStruggling`
- **眨眼**：每 ~3 秒短暫閉眼 0.2 秒（眼睛壓扁成細線）

### 📊 v3.15.0 數字結算

| 指標 | v3.14.0 | **v3.15.0** | 變化 |
|---|---|---|---|
| 主題數量 | 3 | **6** | +3 |
| bgLayer kinds | 4（solid/gradient/stars/bubbles）| **6** | +2 |
| bananaTrail 主題化 | ❌ | ✅ | new |
| Sprite idle/blink | ❌ | ✅ | new |
| 單元測試 | 85 | **94** | +9 |
